    ...
    if (payload == null || !('Name' in payload)) {
      ...
      if (response == null) {
        ...
        if (response.Header.Action === 'http://schemas.xmlsoap.org/ws/2004/09/enumeration/DeleteResponse') {
          ...
          MqttProvider.publishEvent('fail', ['AMT_AlarmOccurrences'], messages.ALARM_OCCURRENCES_DELETE_REQUEST_NOT_FOUND, guid)
          // Use 404 for the http status
          res.status(404).json(ErrorResponse(404, `${messages.ALARM_OCCURRENCES_DELETE_REQUEST_FAILED} for guid : ${guid}.`, 'alarm'))
        }
      }
    }
  } catch (error) {
